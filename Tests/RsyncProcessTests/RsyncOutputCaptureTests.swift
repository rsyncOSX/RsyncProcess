...
struct RsyncOutputCaptureTests {
    ...
    func testDoesNotCaptureWhenDisabled() async {
        let capture = RsyncOutputCapture.shared
    ...
    func testPrintLinesClosure() async {
        let capture = RsyncOutputCapture.shared
        ...
        #expect(lines.count == 2)
        if lines.count >= 2 {
            #expect(lines.contains("Closure line 1"))
            #expect(lines.contains("Closure line 2"))
        }
    ...
    func testProcessHandlersIntegration() async {
        let capture = RsyncOutputCapture.shared
        ...
        let handlers = ProcessHandlers(
            ...
            rsyncversion3: true,
            environment: nil,
            printlines: capture.makePrintLinesClosure()
        )
        
        // Simulate calling printlines
        if let printlines = handlers.printlines {
            printlines("Test output 1")
            printlines("Test output 2")
        }
        ...
        if capturedLines.count >= 2 {
            #expect(capturedLines[0] == "Test output 1")
            #expect(capturedLines[1] == "Test output 2")
        }
